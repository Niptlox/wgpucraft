...
use crate::terrain_gen::chunk::{CHUNK_AREA, CHUNK_Y_SIZE, Chunk, ChunkManager};
use crate::{
    render::{
        atlas::Atlas,
        model::DynamicModel,
...
impl TerrainGen {
    pub fn new(renderer: &Renderer, render_distance_chunks: usize, seed: u32) -> Self {
        ...
        for x in 0..chunk_capacity {
            ...
            let mut chunk_model = DynamicModel::new(
                &renderer.device,
                (CHUNK_AREA * CHUNK_AREA) * CHUNK_Y_SIZE * 24,
            );
    ...
    pub fn load_empty_chunks(&mut self, queue: &Queue) {
        ...
        let chunks_to_update: Vec<usize> = self
            .chunk_indices
            .read()
            .unwrap()
            .iter()
        ...
        chunks_to_update.into_par_iter().for_each(|i| {
            ...
            if let Some(new_index) = new_index {
                ...
                let mut chunk = chunk.write().expect("Error: Failed to lock chunk");
